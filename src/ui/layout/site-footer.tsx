...
export function SiteFooter({ locale, messages }: SiteFooterProps) {
  return (
    <footer className="relative border-t border-white/[0.08] bg-[rgba(4,6,11,0.38)] backdrop-blur-xl">
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.02),transparent_24%)] pointer-events-none" />
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-10 px-6 py-10 sm:px-8 lg:px-10 lg:py-12">
          ...
          <div className="flex max-w-2xl flex-wrap gap-x-6 gap-y-3 text-sm reveal-up reveal-delay-1">
            {siteConfig.navigation.footer.map((item) => (
              <Link
                ...
                className="text-white/62 transition duration-300 hover:text-ink"
              >
